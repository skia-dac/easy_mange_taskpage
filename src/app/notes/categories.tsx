...
import { AppText, Card, IconBadge, ListRow, SubjectDot } from '@/shared/ui';
      ...
      <Card>
        {(categories.data ?? []).map((c) => (
          <ListRow
            ...
            onPress={() => router.push({ pathname: '/notes/category-form', params: { id: c.id } })}
          />
        ))}
        <ListRow
          title={t('noteCategories.new')}
          leading={<IconBadge icon="plus" />}
          onPress={() => router.push('/notes/category-form')}
        />
      </Card>
    </ScrollView>
  );
