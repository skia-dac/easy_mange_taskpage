...
- **Local schema**: migration **17** (16: loan currency, unique course exceptions; 17: local `feedback` table, §5k). Server SQL regenerated: re-run `supabase/migrations/20260924000000_mysky.sql` on the project after pulling.
...
## 5k. Retours des utilisateurs — « Donner mon avis » (built 26 Sep 2026)

- **Where**: Profil › « Donner mon avis » (`/feedback`) and « Mes retours » (`/feedback/history`, shown once a feedback exists); the root `ErrorBoundary` offers « Signaler ce problème » (`/feedback?kind=bug&area=other&error=<error name>`, the error *name* only, never its message). Route params `kind` and `area` pre-fill the form.
- **One screen**: type (3 radio cards, required), part of the app (chips; Argent only with Perso, Révisions only with Études; default « Autre »), message (10–2000, counter, placeholder per type), « Ça m'empêche d'utiliser l'app » for a bug, optional screenshot (gallery, copied to `attachments/feedback/`), optional contact email (pre-filled with the account's), and a transparent box listing what is attached (MySky version, OS + version, app language). Nothing personal.
- **Data**: local table `feedback` (migration 17), **outside `SYNCED_TABLES`, the outbox and the local backup** (`LOCAL_ONLY_TABLES`), wiped by « Supprimer toutes mes données ». Module `platform/feedback` (`feedbackInputSchema`, `createFeedback`, `listFeedback`, `deleteFeedback`, `sendPendingFeedback`, `submitFeedback`).
